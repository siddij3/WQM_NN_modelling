...
def get_dict(tt_feats):
    dict = {
        ...
        'pH Elapsed':train_features[:, 2] ,
        'Temperature':train_features[:, 3], 
        'Rinse':train_features[:, 4] 
    }
...
if __name__ == '__main__':
    ...
    print("Isolating Time")
    R_pH , mae_averages_pH  = isolateParam(optimal_NNs , all_features, 'pH Elapsed', param_batches, vbs, str_reg )
    # R_time_testdata, mae_averages_time_testdata = isolateParam(optimal_NNs , test_dataset, 'Time',param_batches, vbs, str_test )

    dict_time = {
        "pH ": [i for i in range(0, 51)],
        "pH:  R"    : R_pH , 
